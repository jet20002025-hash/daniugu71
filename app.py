...
SCAN_MODE_OPTIONS: List[Tuple[str, str]] = [
    ...
    ("mode38", "mode38（大牛股关键位回踩）"),
    ("mode39", "mode39（大阳锚点回踩再升）"),
    ("mode40", "mode40（新高回调踩60线回升）"),
    ("mode3", "71倍原版(mode3)"),
]
...
def _load_market_caps(path: str) -> Dict[str, float]:
    ...
    with open(path, "r", encoding="utf-8") as handle:
        ...
        for row in reader:
            ...
            total_cap = row.get("total_cap")
            if total_cap is None or total_cap == "":
                total_cap = row.get("market_cap")
            cap_value = float_cap if float_cap not in (None, "") else total_cap
            try:
                cap = float(cap_value)
            except Exception:
                continue
...
def run_mode3_scan(
    ...
    try:
        ...
        results = scan_with_mode3(
            ...
            use_mode93=use_mode93,
            use_mode_bottom_big_yang=use_mode_bottom_big_yang,
            use_mode_platform_breakout_first_yang=use_mode_platform_breakout_first_yang,
            use_mode_mid_big_yang=use_mode_mid_big_yang,
            use_mode_bottom_support=use_mode_bottom_support,
            ...
            use_mode36=use_mode36,
            use_mode37=use_mode37,
            use_mode38=use_mode38,
            use_mode39=use_mode39,
            use_mode40=use_mode40,
        )
...
def scan():
    ...
    use_71x_standard = mode in ("mode3", "mode8", "mode9", "mode10", "mode11", "mode12", "mode90", "mode93", "mode底部大阳线", "mode平台突破首阳", "mode中位大阳线", "mode底部支撑", "mode最后震仓", "mode98", "mode32", "mode34", "mode35", "mode36", "mode37", "mode38", "mode39", "mode40")
    is_paid = (
        g.current_user.is_activated and not getattr(g.current_user, "subscription_expired", True)
        or getattr(g.current_user, "is_super_admin", False)
        or (getattr(g.current_user, "username", "") == "superzwj")
