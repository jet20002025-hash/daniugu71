...
最终 K 线侧 `score` 由 `_score_mode9` 给出；**扫描结果**还会叠加上下文中的行业趋势与板块资金调整后再与 `min_score` 比较。

---

## 三、板块资金净额（同花顺，扫描阶段）

当 `data/gpt/akshare_cache/sector_flow_ths_features.json` 存在，且其中 **`trade_date` 与信号日 `signal_date` 同为 YYYY-MM-DD** 时（防未来数据，见 `app/sector_trend.merge_ths_flow_features`），按个股所属行业匹配同花顺行业榜中的 **净额（亿元）**：

- **净流入（正）→ 加分；净流出（负）→ 减分**。
- 优先用 **即时** 窗口净额（`ths_flow_net_1d`），若无则用 **5 日排行** 净额（`ths_flow_net_5d`）。
- **默认规则**：每 **3 亿**净额约 **±1 分**（四舍五入），单次扫描内该项对总分的变动 **不超过 ±5 分**（`scan_with_mode3` 参数 `sector_fund_flow_max_points` / `sector_fund_flow_yi_per_point`）。
- **环境变量**（网页扫描与 `app.py` 一致）：`SECTOR_FUND_FLOW_MAX_POINTS`（设为 `0` 可关闭）、`SECTOR_FUND_FLOW_YI_PER_POINT`（默认 `3`）。
- **mode90** 与 mode9 一样会在扫描时应用上述板块资金加减分。

特征文件生成：`python3 scripts/build_ths_sector_flow_features.py --trade-date YYYY-MM-DD`（须与当日扫描信号日一致）。

metrics 中会多出（有调整时）：`sector_fund_flow_net_yi`、`sector_fund_flow_score_delta`。
