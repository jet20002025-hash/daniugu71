...
用法:
  python3 scripts/scan_mode37_period.py --start 2026-04-01 --end 2026-04-30
  python3 scripts/scan_mode37_period.py --code 301348 --start 2026-01-01 --end 2026-06-30
"""
...
from app.mode37_gap_support import (
    MODE37_DISPLAY_NAME,
    mode37_kw_from_scan_config,
    mode37_signal_metrics,
    score_mode37_gap_support,
)
...
def main() -> None:
    ap = argparse.ArgumentParser(description=f"{MODE37_DISPLAY_NAME} 区间扫描")
    ...
    kw = mode37_kw_from_scan_config(ScanConfig(min_score=args.min_score))
    ...
    for item in stock_list:
        ...
        for d in trade_days:
            ...
            if idx is None:
                continue
            score = score_mode37_gap_support(rows, idx, code, name, **kw)
            if score < args.min_score:
                continue
            m = mode37_signal_metrics(rows, idx, code, name, **kw)
            hits.append(
                {
                    "signal_date": d,
                    "code": code,
    ...
    hits.sort(key=lambda x: (-x["score"], x["signal_date"], x["code"]))
    default_out = os.path.join(
        GPT_DATA_DIR,
        "results",
        f"mode37_gap_support_{start_ymd[:7].replace('-', '_')}.csv",
    )
    ...
    print(f"命中 {len(hits)} 条 → {out_path}")
    for h in hits[:20]:
        print(
            f"  {h['signal_date']} {h['code']} {h['name']} "
            f"分{h['score']} [{h.get('signal_type','')}] "
            f"缺口{h.get('gap_date')} {h.get('gap_bottom')}-{h.get('gap_top')} "
            f"回踩{h.get('touch_dist_pct')}% 次{h.get('touch_count')}"
        )
